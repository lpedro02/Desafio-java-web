...
public class EventoController {
	...
	public ResponseEntity<Void> insert(@RequestBody EventoNewDto eventonewdto) throws ParseException {
	...
	ResponseEntity<List<EventoResponseDto>> findall() {
		...
		List<EventoResponseDto> list_eventos_responsedto = eventoservice
				.List_EventotoList_EventResponseDto(eventoservice.FindAll());
	...
	@PutMapping("/userattheevent/{nome_evento}")
	@ApiOperation(value = "Inserir um usuario em um Evento")
	public ResponseEntity<Void> Update (@PathVariable String nome_evento,  String nome_usuario)
			throws ObjectNotFoundException {

		Evento evento = eventoservice.EventoaddUser(nome_evento, nome_usuario);

		URI uri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{nome}").buildAndExpand(evento.getId())
				.toUri();
		return ResponseEntity.created(uri).build();
